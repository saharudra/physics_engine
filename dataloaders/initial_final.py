...
class InitialFinal(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for obj in range(num_objects):
            curr_obj_inital_mask = torch.from_numpy(list(curr_masks)[obj][0])
            print("curr object initial mask shape")
            print(curr_obj_inital_mask.shape)
            curr_obj_final_mask = torch.from_numpy(list(curr_masks)[obj][1])
    ...
    def __len__(self):
        if self.partition == 'train':
            return (len(os.listdir(self.params['data_root'] + '/train/')) - 1) // self.params['num_files']
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        initial_image = torch.from_numpy(initial_image).float().div(255)
        final_image = torch.from_numpy(final_image).float().div(255)
        sample['ini_img'] = initial_image
        sample['fin_img'] = final_image
        return sample
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='/Users/i23271/Downloads/PhD/o2p2/physics_engine/configs/pre-planning.yml',
    ...
    for idx, sample in enumerate(train_loader):
        ...
        plt.imsave('ini_img.png', ini_img)
        plt.imsave('fin_img.png', fin_img)
        for obj in range(num_objects):
            plt.imsave('ini_masks_' + str(obj) + '.png', ini_masks[obj])
            plt.imsave('fin_masks_' + str(obj) + '.png', fin_masks[obj])
